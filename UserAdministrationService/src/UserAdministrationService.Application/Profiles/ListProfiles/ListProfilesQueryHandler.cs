...
{
    public async ValueTask<Result<ListProfilesResult>> Handle(ListProfilesQuery query, CancellationToken cancellationToken)
    {
        User? user = await usersRepository.GetByIdAsync(query.UserId, cancellationToken);

        if (user is null)
        {
            return Result.Failure<ListProfilesResult>(Error.NotFound(description: "User not found", code: "UserNotFound"));
        }
