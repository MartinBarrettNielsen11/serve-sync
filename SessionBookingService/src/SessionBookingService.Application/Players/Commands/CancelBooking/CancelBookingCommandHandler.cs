    ...
    : IRequestHandler<CancelBookingCommand, Result>
{
    public async ValueTask<Result> Handle(CancelBookingCommand command, CancellationToken cancellationToken)
    {
        ...
        {
            return Result.Failure<Result>(Error.NotFound(description: "User does not have a reservation for the given session", code: "BLablbla"));
        }
        ...
        {
            return Result.Failure<Result>(Error.Unexpected(description: "Participant expected to have reservation to session", code: "BLablbla"));
        }
