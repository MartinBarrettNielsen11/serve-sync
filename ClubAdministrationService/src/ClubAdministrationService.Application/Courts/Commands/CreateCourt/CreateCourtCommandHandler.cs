	...
	: IRequestHandler<CreateCourtCommand, Result<Court>>
{
	...
	{
		Club? club = await clubsRepository.GetByIdAsync(
			command.ClubId,
			cancellationToken);

		if (club is null)
		{
			return Result.Failure<Court>(
				Error.NotFound("ClubNotFound", "Club not found"));
		}

		Subscription? subscription =
			await subscriptionsRepository.GetByIdAsync(
				club.SubscriptionId,
				cancellationToken);
